...
class Alert3OUHalfLogger:
    ...
    def log_fetch_qualifying_only(self, filtered_matches, fetch_id, nyc_timestamp):
        ...
        with open('mock_alert_3ou_half.json', 'a') as f:
    ...
    def rotate_log(self):
        ...
        self.setup_logging()
        # Clear the main mock_alert_3ou_half.json file
        open('mock_alert_3ou_half.json', 'w').close()
...
class Alert3OUHalfProcessor:
    ...
    def process_monitor_data(self, monitor_data_path):
        """MOCK VERSION: Filter matches: Status 3 (Half-time), 0-0 score, O/U Total >= 3.0"""
        
        # MOCK: Read directly from mock monitor_central.json (no tracking system needed)
        try:
            with open(monitor_data_path, 'r') as f:
                mock_data = json.load(f)
        except Exception as e:
            print(f"Error reading mock monitor data: {e}")
            return {"error": f"Could not read mock data: {e}"}
        
        # Generate mock fetch ID for logging
        import random
        import string
        fetch_id = "MOCK_" + ''.join(random.choices(string.ascii_letters + string.digits, k=8))
        
        try:
            # Filter matches based on criteria
            filtered_matches = self.filter_matches(mock_data.get("monitor_central_display", []))
            ...
            # MOCK: Skip tracking file operations and underdog alert triggering
            print(f"🧪 MOCK: Processed {len(filtered_matches)} qualifying matches")
            
            return filtered_data
        except Exception as e:
            print(f"Error processing mock alert_3ou_half data: {e}")
            ...
            for section in fetch_sections[1:]:  # Skip first empty section
                try:
                    ...
                    if json_start != -1 and json_end != -1:
                        ...
                        # MOCK: Handle clean logging format with multiple JSON objects
    ...
    def get_alerted_match_ids_persistent(self):
        ...
        alerted_file = 'mock_alerted_match_ids.txt'
        try:
...
if __name__ == "__main__":
    ...
    if single_run_mode:
        ...
        try:
            result = alert_3ou_half_processor.process_monitor_data("../mock_6_monitor_central/mock_monitor_central.json")
            print(f"Single alert_3ou_half completed! Logged to: mock_alert_3ou_half.json")
        except Exception as e:
    ...
    def continuous_alert_3ou_half():
        ...
        while running:
            ...
            try:
                ...
                print(f"[Fetch #{fetch_count}] Logged to: mock_alert_3ou_half.json")
